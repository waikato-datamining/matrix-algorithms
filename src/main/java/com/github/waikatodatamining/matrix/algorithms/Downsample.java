...
public class Downsample
  ...
  public void setStartIndex(int value) {
    if (value < 0) {
      getLogger().warning("Start index must be at least zero, was " + value + ".");
    } else {
      m_StartIndex = value;
    }
  }
  ...
  public void setStep(int value) {
    if (value < 1) {
      getLogger().warning("Step must be at least 1, was " + value + ".");
    } else {
      m_Step = value;
    }
  }
  ...
  public Matrix doTransform(Matrix predictors) {
    ...
    if (m_StartIndex >= predictors.numRows()) {
      throw new IndexOutOfBoundsException("Start index (" +
        m_StartIndex +
